...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, api_key, self_refinement, num_prompt, max_rounds,frame,stage):
    first_iter = 0
    tb_writer = prepare_output_and_logger(dataset)
    gaussians = GaussianModel(dataset.sh_degree)
    scene = Scene(dataset, gaussians, api_key, self_refinement, num_prompt, max_rounds,frame=frame,stage=stage,load_iteration=checkpoint)
    ...
    for iteration in range(first_iter, opt.iterations + 1):   
        ...
        gt_image=gt_image.unsqueeze(0).cuda()
        image=image.unsqueeze(0).cuda()
        
      
      
        lpips_result=lpips_loss(gt_image,image).squeeze()
        
        loss+=lpips_result*0.05

        if iteration<30000:
            if iteration%10000 > 5400 and iteration%10000 <= 9000:
                if iteration%10000 > 5400 and iteration%10000 <= 6600:
                ...
                elif iteration%10000 > 6600 and iteration%10000 <= 7800:
                    if not perturbation_viewpoint_stack:
                        perturbation_viewpoint_stack = scene.getPerturbationCameras(stage=2).copy()
                    perturbation_viewpoint_cam = perturbation_viewpoint_stack.pop(randint(0, len(perturbation_viewpoint_stack)-1))
                elif iteration%10000 <= 9000:
                    if not perturbation_viewpoint_stack:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start_checkpoint", type=int, default = None)
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.api_key, args.self_refinement, args.num_prompt, args.max_rounds,args.frame,args.stage)
