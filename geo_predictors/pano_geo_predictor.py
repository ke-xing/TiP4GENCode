...
class PanoGeoPredictor(GeoPredictor):
    ...
    def __call__(self, img, gen_res=512, reg_loss_weight=1e-1,dust_rot=None):
        '''
        :param img: [H, W, 3]
        ...
        for i in range(12):
            ratio = 1.1
            cur_pers_dirs, cur_pers_ratios, cur_to_vecs, cur_down_vecs, cur_right_vecs = panorama_to_pers_directions(gen_res=gen_res, ratio=ratio, ex_rot='rand',dust_rot=dust_rot)
        ...
        scale_params = torch.zeros([n_pers_pc], requires_grad=True,device='cuda')
        bias_params_local_distance  = torch.zeros([n_pers_pc, 1, gen_res, gen_res], requires_grad=True,device='cuda')
        ...
        for phase in ['global', 'hybrid']:
            ...
            progress_bar = tqdm.tqdm(range(1, all_iter_steps + 1), desc="Training progress")
            loss_vis = []
            ###
