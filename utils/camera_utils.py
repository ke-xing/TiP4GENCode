...
def loadCam(args, id, cam_info, resolution_scale,scale,shift):
    ...
    min_thre = min(1e-6, np.quantile(aligned_depth, 0.01))
    aligned_depth[aligned_depth < min_thre] = 0.0
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  ...
                  image_name=cam_info.image_name, uid=id, data_device=args.data_device, depth_image=aligned_depth)

def cameraList_from_camInfos(cam_infos, resolution_scale, args,scale=1,shift=0):
    camera_list = []
    ...
    for id, c in tqdm.tqdm(enumerate(cam_infos)):
        camera_list.append(loadCam(args, id, c, resolution_scale,scale=scale,shift=shift))
