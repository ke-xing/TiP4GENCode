...
def save_data(source_path, pano_img, distances, R, fx, fy, cx, cy, pers_imgs, pts,frame,stage):
    ...
    pts_folder = os.path.join(os.path.dirname(source_path), 'pts')
    ...
    if stage==1:
        ...
        with open(images_file_path, 'w') as f:
            ...
            for i, R_mat in enumerate(R):
                ...
                quaternion = Rotation.from_matrix(R_mat[:3,:3]).as_quat()  # [x, y, z, w]
                # Assuming placeholders for TX, TY, TZ
                tx, ty, tz = R_mat[:3,3] # Replace these with your actual data if available
                # Write to images.txt
    ...
    if frame==0 and stage==0:
        # Define the path for points3D.txt
        points3D_file_path = os.path.join(pts_folder, 'points3D.txt')
        ...
        pcd_path = os.path.join(pts_folder, 'points3D.ply')
        pcd.export(pcd_path)

        print('Saved perspective images to ', image_folder)
        print('Saved camera poses to ', pts_folder)
